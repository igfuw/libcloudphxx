...
rain_cover  = {"out_hall_pinsky_stratocumulus" : 0}
cloud_cover = {"out_hall_pinsky_stratocumulus" : 0}
...
for kernel in kernels:
    ...
    for run in {"seed_44"}: #{"seed_9", "seed_13", "seed_30", "seed_42", "seed_44"}:
        ...
        cloud_mixr_fin = h5f_fin['rw_rng000_mom3'][:] * 4./3 * 3.14 * 1e3 * 1e3 #g/kg
    
        idx  = np.where(rain_mixr_fin  >= cutoff)
        idx2 = np.where(cloud_mixr_fin >= cutoff)

        rain_cover[kernel]  += idx[0].size
        cloud_cover[kernel] += idx2[0].size
        n_run += 1.
        ...
        for i in range(num_dry-1): # first bin id for total conc
            ...
            n_dry_cld[i] += tmp_fin[idx2].sum() / idx2[0].size  / (mt.log(dry_edges[i+1], 10) - mt.log(dry_edges[i], 10))
    ...
    cloud_cover[kernel] = cloud_cover[kernel] / n_run / 76 / 76 * 100 
    print kernel, "  ", rain_cover[kernel]
    print kernel, "  ", cloud_cover[kernel]
    ...
    g.plot(plot_rd_ini, plot_rd_cld, plot_rd_fin)
    ...
    g.plot(plot_rw_rain, plot_rw_all)
