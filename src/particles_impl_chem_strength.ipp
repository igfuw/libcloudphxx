  ...
  namespace lgrngn
  {
    namespace detail
    {
      ...
      { // helper for choosing which droplets are dilute and can have chemical reactions
        ...
        BOOST_GPU_ENABLED        
        unsigned int operator()(const thrust::tuple<real_t, real_t, real_t, real_t, real_t, real_t, real_t, real_t> &tpl) const 
        {
          ...
          return 
            ...
            ) * si::cubic_metres / si::moles) < 0.1 * 1000 ? 1 : 0;
        }                                          // ^^^^ convert from moles/litr to moles/m3
      };
    };
