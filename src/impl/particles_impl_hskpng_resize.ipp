namespace libcloudphxx
{
  namespace lgrngn
  {
    ...
    void particles_t<real_t, device>::impl::hskpng_resize_npart()
    {
      ...
      {
        thrust_device::vector<real_t> *vec[] = {&rw2, &rd3, &kpa, &vt, &tmp_device_real_part, &delta_revp20, &delta_revp25, &delta_revp32, &delta_accr20, &delta_accr25, &delta_acnv20, &delta_acnv25, &delta_accr32, &delta_acnv32};
        for(int i=0; i<7; ++i)
        {
          vec[i]->resize(n_part);
        }
