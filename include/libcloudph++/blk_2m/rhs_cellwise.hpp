...
//</listing>
    {  
      ...
      ))
      {
        ...
        const quantity<si::dimensionless, real_t> &rc    = boost::get<7>(tup) * si::dimensionless();
        const quantity<
          divide_typeof_helper<si::dimensionless, si::mass
        >::type,                          real_t> &nc    = boost::get<8>(tup) / si::kilograms;
        ...
        { //TODO what if we have some other source terms (that happen somewhere before here), like diffusion?
          ...
          if (rv > common::const_cp::r_vs<real_t>(T, p))
          {
            ...
            quantity<divide_typeof_helper<si::frequency, si::mass>::type, real_t> tmp = 
              activation_rate<real_t>(n_ccn, nc, dt * si::seconds);
        ...
        {                          
          ...
          {          //  ^^   TODO is it possible?
            ...
            {   // so that we don't evaporate more cloud water than there is
              ...
              dot_nc  = -nc / dt * si::kilograms; // and all nc
            }
      ...
      ))
      {
        ...
        const quantity<si::mass_density, real_t>  &rhod = boost::get<4>(tup) * si::kilograms / si::cubic_metres;
        const quantity<si::dimensionless, real_t>   &rc = boost::get<5>(tup) * si::dimensionless();
        const quantity<divide_typeof_helper<si::dimensionless, si::mass>::type, real_t>  
...
//        }
      ...
      for (auto tup : zip(
        ...
        const quantity<si::dimensionless, real_t> &rr    = boost::get<7>(tup) * si::dimensionless();
        const quantity<divide_typeof_helper<si::dimensionless, si::mass>::type, real_t>  
                                                  &nr    = boost::get<8>(tup) / si::kilograms;
        ...
        if (opts.cond)
        {
          ...
          { // cond/evap for rr
            ...
            assert(nr * si::kilograms + dot_nr * dt >= 0 && "before rain cond-evap");
            assert(th / si::kelvin + dot_th * dt >= 0 && "before rain cond-evap");

            quantity<si::frequency, real_t> tmp = 
              cond_evap_rate<real_t>(T, p, rv, tau_relax_r(T, rhod, rr, nr));

            assert(r_drop_r(rr, nr) >= 0 * si::metres  && "mean drop radius cannot be < 0");
            ...
            if (rr + (dot_rr / si::seconds + tmp) * (dt * si::seconds) < 0) // so that we don't evaporate more than we have
            {
              ...
              dot_nr  = -nr / dt * si::kilograms; // and all n_r
            ...
            else
            {
              ...
              if (tmp < 0 / si::seconds) 
              {
                quantity<divide_typeof_helper<si::frequency, si::mass>::type, real_t> dot_nr_tmp = tmp * nr / rr;

                if (nr + (dot_nr / si::kilograms / si::seconds + dot_nr_tmp) * (dt * si::seconds) > 0 / si::kilograms)
                {
